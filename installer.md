...
EC2에 올라가는 application은 **FastAPI + React Web UI**와 같은 프로세스의 LangGraph Agent로 구성됩니다. Node.js, MCP, LangGraph 패키지를 포함하며, 실제 빌드 정의는 저장소 루트의 [`Dockerfile`](./Dockerfile)을 사용합니다.
...
docker run -d --restart=always -p 8501:8501 \
  ...
  --name app agent-wiki-app
```

헬스체크: `GET /api/health` · 진입점: `uvicorn application.server:app --host 0.0.0.0 --port 8501`
